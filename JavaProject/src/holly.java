...
class MainFrame extends JFrame implements MouseListener, Runnable { //메인프레임클래스

    private JLabel lb_title = new JLabel();//게임타이틀표시용라벨
    private JLabel lb_time = new JLabel();//시간표시용라벨
    private JButton bt_start = new JButton("START");//게임시작버튼
    private JButton bt_reset = new JButton("RESET");//게임리셋버튼
    ...
    public void init() {//게임라벨, 버튼및게임화면용패널설정
        ...
        con.add(bt_reset);//프레임에표시할게임시작및리셋버튼
        sc = new ImagePanel();
    ...
    public void start() {//프레임내실행시킬기본내용
        ...
        lb_title.setText("1 to 50 Game");//라벨텍스트내용설정
    ...
    public void mouseClicked(MouseEvent e) {
        if (e.getSource() == bt_start) {//게임시작버튼
            if (!time_run && !sc.game_start ) {
                ...
                time_run = true;
                sc.gameStart(true);//게임및시간세팅
                //System.out.println("start");
            }
        }
        else if (e.getSource() == bt_reset) {//게임초기화버튼
            start_time = 0;
            lb_time.setText("00:00:00.000");
            sc.rect_select.clear();
            sc.countDown(0);
...
class ImagePanel extends JPanel implements MouseListener {//실제게임화면을표시할패널
